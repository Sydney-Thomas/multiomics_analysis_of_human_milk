...
names <- c("Secretor", "C_section", "ABX_birth", "ABX_pregnancy", "EBF", "Siblings", "Pets", "AB_1M", "AB_6M",
           "AB_12M", "Probiotic_1M", "Probiotic_6M", "Probiotic_12M")
...
norm <- read.csv("./Data/ES/Collado_iimn_gnps_quant.csv")
...
canopus <- read.delim("./Data/ES/canopus_compound_summary.tsv")
...
norm_iimn <- norm_iimn %>%
  gather(HMO_ID, value, 2:ncol(norm_iimn)) %>%
  spread(row.ID, value)
names <- metadata %>% dplyr::select(SampleID, HMO_ID)
norm_iimn$HMO_ID <- str_replace_all(norm_iimn$HMO_ID, "\\.", "-")
norm_iimn$HMO_ID <- str_replace_all(norm_iimn$HMO_ID, "^X", "")
norm_iimn <- right_join(names, norm_iimn, by = "HMO_ID") %>% dplyr::select(-HMO_ID) %>% filter(SampleID != "")
norm_iimn <- column_to_rownames(norm_iimn, var = "SampleID")
...
norm_iimn <- rownames_to_column(norm_iimn, var = "SampleID")
set.seed(141)
norm_imp <- norm_iimn
...
normalized <- read.csv("./Data/ES/Metabolites_normalized_imputed.csv")
library_ID <- read.csv("./Data/ES/library_matches.csv")
canopus <- read.csv("./Data/ES/canopus_matches.csv")
data <- left_join(normalized, metadata, by = "SampleID")
...
groups <- data %>% 
  dplyr::select(SampleID, Amigos, Infant_age_days, starts_with("X")) %>% 
  filter(!is.na(Amigos))
...
# Run tukey_HSD
pval <- groups %>% pivot_longer(5:ncol(groups), names_to = "Metabolite", values_to = "Abundance")
pval <- pval %>% dplyr::select(-SampleID) %>% group_by(Metabolite) %>% tukey_hsd(Abundance~Amigos)
...
data <- left_join(normalized, metadata, by = "SampleID")
corr <- data %>% dplyr::select(SampleID, z6DDL, starts_with("X")) 
corr <- column_to_rownames(corr, var = "SampleID")
...
##########################################################################################################################################################
####################### CANOPUS ##########################################################################################################################
data <- read.csv("./Data/ES/Metabolites_normalized.csv")
data <- data %>% 
...
data <- canopus_all %>% 
  dplyr::select(-Type)
data <- data %>% 
  gather(SampleID, value, 2:ncol(data)) %>% 
  spread(canopus, value)
data <- left_join(data, metadata, by = "SampleID")
corr <- data %>% dplyr::select(SampleID, z6DDL, 2:Xanthines_Classy) 
corr <- column_to_rownames(corr, var = "SampleID")
cor <- rcorr(as.matrix(corr), type=c("spearman"))
