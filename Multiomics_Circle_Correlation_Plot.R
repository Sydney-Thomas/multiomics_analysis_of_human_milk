...
metabolites_ES <- read.csv("./Data/ES/Metabolites_normalized_imputed.csv")

## Read in microbiome data
## Data is preprocessed using rclr and annotated at genus level
microbiome_US <- read.csv("./Data/US/Microbiome_table_SEED_genus_rclr.csv")
microbiome_ES <- read.csv("./Data/ES/Microbiome_table_Collado_genus_rclr.csv")
...
microbiome_ES <- microbiome_ES %>% dplyr::select(SampleID, contains("othia"), contains("eillon"))
...
ES <- inner_join(metabolites_ES, microbiome_ES, by = "SampleID")
ES <- inner_join(HMO, ES, by = c("Sample" = "SampleID"))
...
# ## Spearman correlation for ES dataset
# ES <- ES %>% dplyr::select(Sample, !!sym(G1), !!sym(G2), !!sym(G3), everything())
# ES <- ES %>% pivot_longer(5:ncol(ES), names_to = "To", values_to = "To_Abundance")
# ES <- ES %>% pivot_longer(2:4, names_to = "From", values_to = "From_Abundance")
# ES_corr <- ES %>% group_by(To, From) %>% rstatix::cor_test(vars=c("To_Abundance"), vars2=c("From_Abundance"), method="spearman")
# 
# ## BH adjESt p values
# ES_corr$padj <- p.adjust(ES_corr$p, method = "BH")
# 
# ## Find metabolites that are significantly correlated with Rothia, Veillonella, and 6DDL
# test <- ES_corr %>%
#   group_by(To) %>%
...
# 
# ## Make table of library and canopus IDs
# lib_ES <- ES_corr %>% dplyr::select(To) %>% unique()
# lib <- read.csv("./Data/ES/library_matches.csv")
# lib$Metabolite <- paste0("X", lib$Metabolite)
# can <- read.csv("./Data/ES/canopus_matches.csv")
# can$Metabolite <- paste0("X", can$Metabolite)
# lib_ES <- right_join(lib, lib_ES, by = c("Metabolite" = "To"))
# lib_ES <- left_join(lib_ES, can, by = "Metabolite")
